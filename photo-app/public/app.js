...
async function checkout(plan){if(me.emailRequired&&!me.emailVerified){setStatus('이메일 인증 후 결제할 수 있습니다.');return;}if(!confirm(`${plan.name} ${plan.price.toLocaleString('ko-KR')}원${plan.id==='pro'?' 매월 자동결제':''} 테스트를 진행할까요? 실제 청구는 없습니다.`))return;
  ...
  if(plan.id==='pack'){const order=await api('/api/checkout',json({plan:plan.id,requestKey:crypto.randomUUID()}));await window.TossPayments(order.clientKey).payment({customerKey:order.customerKey}).requestPayment({method:'CARD',amount:{value:order.amount,currency:'KRW'},orderId:order.id,orderName:'사진꾸러미 충전팩',customerName:order.customerName,successUrl:location.origin+'/',failUrl:location.origin+'/?paymentFailed=1'});}
  else{const subscription=await api('/api/subscription/start',{method:'POST'});await window.TossPayments(subscription.clientKey).payment({customerKey:subscription.customerKey}).requestBillingAuth({method:'CARD',customerName:subscription.customerName,successUrl:location.origin+'/',failUrl:location.origin+'/?paymentFailed=1'});}
}
